...
use crate::encoder::image::GenericImageView;
...
static mut IMAGE_WIDTH: u32 = 320;
static mut IMAGE_HEIGHT: u32 = 240;

impl Encoder {
    pub unsafe fn new(image: DynamicImage, rate: u32) -> Self {
    ...
    pub unsafe fn encode(self: &mut Self) -> Result<Vec<i16>, String> {
        self.add_vis_header();
        self.add_codificated_image();
        Ok(self.buffer.clone())
    }

    unsafe fn add_codificated_image(self: &mut Self) {
        let mut y = 0;
    ...
    unsafe fn add_y_scan(self: &mut Self, y: &u32) {
    ...
    unsafe fn add_v_scan(self: &mut Self, y: &u32) {
    ...
    unsafe fn add_u_scan(self: &mut Self, y: &u32) {
        for ticks in 0..self.uv_ticks {
            let x0: u32 = self.fclampf(160.0 * ticks as f32 / self.uv_ticks as f32, 0.0, 159.0) as u32;
            let x1: u32 = self.fclampf((x0 + 1) as f32, 0.0, 159.0) as u32;
